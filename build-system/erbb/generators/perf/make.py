...
class Make:
   ...
   def generate_module (self, path, module, strict):
      ...
      path_perf = os.path.join (path, 'artifacts', 'perf')
      path_makefile = os.path.join (path_perf, 'Makefile')

      if not os.path.exists (path_perf):
         os.makedirs (path_perf)

      with open (path_template, 'r', encoding='utf-8') as file:
         template = file.read ()

      path_root = os.path.relpath (PATH_ROOT, path_perf)
      path_libdaisy = os.path.relpath (PATH_LIBDAISY, path_perf)
      ...
      path_lds = os.path.relpath (lds_path, path_perf)
      ...
      template = self.replace_bases (template, module, module.bases, path_perf);
      template = self.replace_sources (template, module, module.sources, path_perf)
      template = self.replace_actions (template, module, path_perf)
   ...
   def replace_bases (self, template, module, bases, path_perf):
      lines = ''
      ...
      if module.source_language == 'max':
         ...
         lines += 'FLAGS += -I%s\n' % os.path.relpath (path_gen_dsp, path_perf).replace ('\\', '/')
         lines += 'FLAGS += -I..\n'
      ...
      for base in bases:
         path_base = os.path.relpath (base.path, path_perf)
         lines += 'FLAGS += -I%s\n' % path_base.replace ('\\', '/')
   ...
   def replace_sources (self, template, module, sources, path_perf):
      lines = ''
      ...
      add_source_path (os.path.join (path_perf, 'main.cpp'))
      ...
      if has_data:
         add_source_path (os.path.join (path_perf, '../plugin_generated_data.cpp'))

      if module.source_language == 'max':
         add_source_path (os.path.join (path_perf, '../%s_erbb.cpp' % module.name))
         add_source_path (os.path.join (path_perf, '../%s_erbui.cpp' % module.name))
         add_source_path (os.path.join (path_perf, '../module_max_alt.cpp'))
      ...
      for source_extra_path in source_extra_paths:
         rel_path = os.path.relpath (source_extra_path, path_perf)
      ...
      for source_path in source_paths:
         rel_path = os.path.relpath (source_path, path_perf)
   ...
   def replace_actions (self, template, module, path_perf):
      lines = ''
      lines += self.replace_actions_max (module, path_perf)
      lines += self.replace_actions_faust (module, path_perf)
      lines += self.replace_actions_ui (module, path_perf)
      lines += self.replace_actions_perf (module, path_perf)
      lines += self.replace_actions_data (module, path_perf)
   ...
   def replace_actions_max (self, module, path_perf):
      if module.source_language != 'max':
         return ''

      lines = ''

      path_erbb_gens = os.path.relpath (PATH_ERBB_GENS, path_perf)
      path_erbui_gens = os.path.relpath (PATH_ERBUI_GENS, path_perf)
   ...
   def replace_actions_faust (self, module, path_perf):
   ...
   def replace_actions_ui (self, module, path_perf):
      lines = ''
   ...
   def replace_actions_perf (self, module, path_perf):
      lines = ''

      path_erbui_gens = os.path.relpath (PATH_ERBUI_GENS, path_perf)

      inputs = os.path.join (path_erbui_gens, 'perf', 'code.py').replace ('\\', '/') + ' '
      inputs += '../../%s.erbui' % module.name
      ...
      lines += '\t@%s ../actions/action_perf.py\n\n' % sys.executable.replace ('\\', '/')
      lines += 'ACTIONS += %s\n\n' % outputs
   ...
   def replace_actions_data (self, module, path_perf):
      data_paths = []
      ...
      if data_paths:
         ...
         for data_path in data_paths:
            inputs += '%s' % os.path.relpath (data_path, path_perf).replace ('\\', '/') + ' '
