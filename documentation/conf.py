...
class ErbuiLexer(RegexLexer):
   ...
   tokens = {
      'root': [
         ...
         (r'[0-9\.]+', token.Literal.Number),
         (r'module|board|width|material|header|footer|line|control|label|sticker|image|pin|pins|cascade|mode|position|rotation|offset|style|positioning|route|faust|address|init|value|property|exclude|extends|alias|file', token.Keyword),
         ...
         (r'AudioIn|AudioOut|Button|CvIn|CvOut|GateIn|GateOut|Led|LedBi|LedRgb|Pot|Switch|Trim', token.Keyword.Type),
         (r'{|}|,', token.Punctuation),
         (r'(?!\b({})\b)([a-zA-Z]\w*)', token.Name.Variable.Class),
         (r'.', token.Text),
      ],
